...
import { AuthType, Chain } from '../../types/types';
...
export class Auth {
  ...
  async addWalletConfirmation(
    ...
    try {
      this.validationService.validateString(walletConfirmationInput.signature);
  ...
  async refreshToken(existingRefreshToken: string) {
    try {
      this.validationService.validateString(existingRefreshToken);
      return (
  ...
  async unregisterAuthMethod(data: string, type: AuthType) {
    try {
      this.validationService.validateString(data);
